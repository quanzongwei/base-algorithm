...
/**
 * 3 数转为2数相加等于第三个数, 再用三层循环,以及二分查找快速定位数据
 * <p/>
 * ArrayList没有二分查找的功能, 没有直接按照对象查索引的功能,
 * 因为这里的数据不保证是排序的,可能会有多个值
 * contains和remove都是顺序往后遍历的
 * <p/>
 * ArrayList的contains是从前往后遍历, get(i)是所及存取
 * LinkedList的contains也是从前往后遍历的,get(i),根据i的大小,决定从后往前还是从前往后,双端队列效率提高一半
 * <p/>
 * 所以arrayList的二分查找或者linkedList的跳表查询需要自己实现
 * <p/>
 * collections.sort(List.toArray())使用的是二分插入排序,如果数据量比较少的话
 * ->list.sort()
 * ->list.toArray()
 ...
 * <p/>
 * Arrays.copyOf(T[], len)
 * Arrays.copyOfRange(T[],from,to) inclusive , exclusive
 * 底层调用System.arrayCopy(src,srcPos,dst,dstPos)
 * <p/>
 * 人家几百毫秒, 几十毫秒
 */
public class A1_三数和为零的问题 {

    /**
     * version: 1
     * 简单粗暴,3层遍历,3000个元素的大数组需要99s
     * <p/>
     * 单个大数据就要99s
     */
    public List<List<Integer>> threeSum(int[] nums) {
        List<List<Integer>> result = new ArrayList<>(3000 * 3000);
        Arrays.sort(nums);
        for (int i = 0; i < nums.length - 2; i++) {//至少移两位
            for (int j = i + 1; j < nums.length - 1; j++) {
                int targetValue = -(nums[i] + nums[j]);
                for (int k = j + 1; k < nums.length; k++) {
                    if (nums[k] == targetValue) {
                        int num2Pos = j;
                        int num3Pos = k;
                        //
                        if (num2Pos >= 0) {//num2 或num3>-1,就表示存在num1+num2+num3=0的数据
                            List<Integer> value = new ArrayList<>();
                            value.add(nums[i]);
                            value.add(nums[num2Pos]);
                            value.add(nums[num3Pos]);
                            if (!result.contains(value)) {
                                result.add(value);
                            }
                            //难道说add是一个耗时操作吗,1000 000 000 10亿,到后面是越来越慢的,指数型增长
                        }
                    }
                }
            }
    ...
    /**
     * version 2: 移除contains和大量的list.add语句
     * <p/>
     * 优化list的contains问题,依然是超时,大数组运行时间5秒一个
     * 但是结果是正确的
     */
    public List<List<Integer>> threeSumRefactor(int[] nums) {
        List<List<Integer>> result = new ArrayList<>(3000 * 3000);
        Arrays.sort(nums);
        nums = truncate(nums);//处理了0的问题,绕过一个5秒检测
        for (int i = 0; i < nums.length - 2; i++) {//至少移两位
            for (int j = i + 1; j < nums.length - 1; j++) {
                int targetValue = -(nums[i] + nums[j]);
                for (int k = j + 1; k < nums.length; k++) {
     ...
     * version 3: k层使用二分查找
     * <p/>
     * 优化: 第三个值用二分查找,第三个值没有必要遍历,因为知道两个之后第三个值就是确定的了
     * <p/>
     * 这就是时间复杂度的意义了,到后面add是一个很耗时的操作
     * <p/>
     * 优化后大数组从5s已经降低到200ms了leetCode总耗时15s降到5s
     */
    public List<List<Integer>> threeSumRefactorBinarySearch(int[] nums) {
        List<List<Integer>> result = new ArrayList<>(3000 * 3000);
        Arrays.sort(nums);
        nums = truncate(nums);//处理了0的问题,绕过一个5秒检测
        printArray(nums);
        //最后一个0的位置
        int oPos = binSearch(nums, 0, 0, nums.length - 1);
        if (oPos <= 0) {
            oPos = -(oPos);
        }
        for (int i = 0; i < oPos; i++) {
            for (int j = i; j < nums.length; j++) {
                ...
                int k = binSearch(nums, kVal, oPos, nums.length - 1);//再优化
                if (k <= 0) {
                    continue;
                }
                if (k <= j) {
    ...
    /**
     * version 4: i层优化一半, k层二分查找的区间可以再压缩,不是简单的[0,len-1]
     * <p/>
     * 但是效果不明显,二分查找本身就很高的性能
     */
    public List<List<Integer>> threeSumRefactorBinarySearch2(int[] nums) {
        List<List<Integer>> result = new ArrayList<>(3000 * 3000);
        Arrays.sort(nums);
        nums = truncate(nums);//处理了0的问题,绕过一个5秒检测
        printArray(nums);
        ...
        for (int i = 0; i < oPos; i++) {
            ...
            for (int j = i; j < nums.length; j++) {
                ...
                int kVal = -(nums[i] + nums[j]);
                //int k = binSearch(nums, kVal, oPos, nums.length - 1);
                int k = binSearch(nums, kVal, begin, end);//在优化
                if (k <= 0) {
                    continue;
                }
                if (k <= j) {
     ...
     * version 5: 优化i层和j层
     * <p/>
     * 再优化,优化i层和j层,j的最大值是-2*nums[i],没啥效果额
     * 减少一半的二分查找影响不大
     */
    public List<List<Integer>> threeSumRefactorBinarySearch3(int[] nums) {
        List<List<Integer>> result = new ArrayList<>(3000 * 3000);
        Arrays.sort(nums);
        nums = truncate(nums);//处理了0的问题,绕过一个5秒检测
        printArray(nums);
        ...
        for (int i = 0; i < oPos; i++) {
            ...
            int tmp = binSearch(nums, -2 * nums[i], 0, nums.length - 1);//在优化
            if (tmp < 0) {
                tmp = -tmp;
            ...
            for (int j = i; j <= tmp; j++) {
                ...
                int kVal = -(nums[i] + nums[j]);
                //int k = binSearch(nums, kVal, oPos, nums.length - 1);
                int k = binSearch(nums, kVal, begin, end);//再优化
                if (k <= 0) {
                    continue;
                }
                if (k <= j) {
    ...
    /**
     * version 6: j层第一次用二分查找,后面有遍历
     * <p/>
     * before[再优化,优化i层和j层,j的最大值是-2nums[i],没啥效果额
     * 减少一半的二分查找影响不大]
     * j层第一次使用二分查找,后面使用遍历,也没有明显变化
     */
    public List<List<Integer>> threeSumRefactorBinarySearch4(int[] nums) {
        List<List<Integer>> result = new ArrayList<>(3000 * 3000);
        Arrays.sort(nums);
        nums = truncate(nums);//处理了0的问题,绕过一个5秒检测
        printArray(nums);
        ...
        for (int i = 0; i < oPos; i++) {
            ...
            for (int j = i; j <= tmp; j++) {
                ...
                //int k = binSearch(nums, kVal, begin, end);//在优化
                int k = -1;
                while (nums[tmpCopy] >= 0) {
                    if (nums[tmpCopy] == kVal) {
                        k = tmpCopy;
                        break;
                    } else if (nums[tmpCopy] <= kVal) {
                        break;
                    }
                    tmpCopy--;
                }
    ...
    /**
     * -2 -2  0 0 1 1 最多是这种情况, 只有0是可能出现3个,其他最多是2个
     * 这种前期优化还是很有效果的(最有的110->85)
     */
    private int[] truncate(int[] nums) {
     ...
     * 可复用
     */
    private boolean exists(int[] nums, int i, int j, int k) {
    ...
    /**
     * 值数组中的值交换,这里还用不到数组copy
     *
     * @param i inclusive
     * @param j inclusive
     */
    private void arrayElementSwap(int[] nums, int i, int j) {
        int tmp = nums[i];
        for (int k = i + 1; k <= j; k++) {
            nums[i - 1] = nums[i];
        }
    ...
    public static void main(String[] args) {
        ...
        List<List<Integer>> result = new A1_三数和为零的问题().threeSumXX(nums);
        System.out.println(result);
        System.out.println(System.currentTimeMillis() - begin);
    }


    public static void mainOrigin(String[] args) {
        long begin = System.currentTimeMillis();
    ...
    /**
     * version 7: 三层遍历到二层,使用双指针, 这个是典型的双指针问题,着来年各个值有关系,相加等于某一个确定的值(一层循环已经确定)
     * <p/>
     * 最简单的方式,最高效最自豪的方式,最长的降到了150ms,leetCode时间是125ms非常nice
     * 这时候就发现时间复杂度的作用了
     */
    public List<List<Integer>> threeSumXX(int[] nums) {
    ...
    /**
     * version 8: 再优化,遍历->二分查找
     * 最优的是65ms左右,战胜84.81%的提交,很棒这才是追求极致性能
     */
    public List<List<Integer>> threeSumXX2(int[] nums) {
        List<List<Integer>> result = new ArrayList<>();
        Arrays.sort(nums);
        nums = truncate(nums);//前期优化,不要把计算量转嫁到深层次的代码中,移除后性能降低,特别对30000个0的情况很有效
        for (int i = 0; i < nums.length - 2; i++) {
            int total = -nums[i];
            int m = i + 1;
            int n = nums.length - 1;
            {//本次优化又更上一层楼,厉害呀 leetCode=87ms,击败65%的代码
                n = binSearch(nums, 2 * total, 0, n);
                if (n <= 0) {
                    n = -n;
                }
                if (n > nums.length - 1 || n < 0) {
                    n = nums.length - 1;
                }
            }
            while (m < n) {
